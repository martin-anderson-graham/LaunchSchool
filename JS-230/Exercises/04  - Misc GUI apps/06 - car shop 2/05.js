...
function updateSelects(make) {

  let optArr = Array.from(document.querySelectorAll('#model > option'));
  console.log(make);
  ...
  let possibleModels = cars.filter(car => car.make === make)
  .map(car => car.model);

  optArr.forEach(opt => {
    if (!possibleModels.includes(opt.textContent)) {
      opt.hidden = true;
    }
  });
}
