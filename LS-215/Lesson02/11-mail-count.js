...
function mailCount(emailData) {
  ...
  let emailList = getEmailList(emailData);

  let parsedEmails = emailList.map( data => parseEmail(data));

  let [minDate, maxDate] = getMinMaxDate(parsedEmails);

  result += `Count of Email: ${emailList.length}\n`;
  result += `Date Range: ${getDateString(minDate)} - ${getDateString(maxDate)}`;
  console.log(result);
}
...
function getEmailList(emailData) {
  return emailData.split(/##\|\|##\n\n/);
}

function parseEmail(emailText) {
  let emailArr = emailText.split(/#\/#\n/);
  return emailArr.reduce( (result, str) => {
    let fieldData = str.split(': ')
    if(fieldData.length === 2) {
      result[fieldData[0]] = fieldData[1];
    } else {
      result['Body'] = fieldData[0]
    }
    return result;
  }, {})
}

function getMinMaxDate(emailObjArr) {
  let dates = emailObjArr.map( emailObj => new Date(emailObj.Date));
  let minDate = dates[0];
  let maxDate = dates[0];
  dates.forEach (date => {
